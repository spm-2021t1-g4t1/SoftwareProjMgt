...
class QuizOptions(db.Model):
    ...
    def get_specificOption(cls, quiz_id, ques_id, opts_id):
        ...
        if data is not None:
            return {
                "data": loads(dumps(data.viewjson(), default=str)),
                "code": 200
            }
        else:
            return {
    ...
    def remove_opt(cls, q_id, quid, oid):
        row_to_delete = cls.query.filter_by(
            quiz_id=q_id, ques_id=quid, opts_id=oid
        ).first()
    ...
    def remove_all_opt(cls, q_id, quid):
        rows_to_delete = cls.query.filter_by(quiz_id=q_id, ques_id=quid)
